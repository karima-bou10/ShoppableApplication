...
export default {
  ...
  methods: {
    // Method to handle saving the collection
    async saveCollection() {
      try {
        // Create a new array with selected information from dotsArr
        const dataToSend = this.dotsArr.map((dot) => ({
          text: dot.text,
          posX: dot.posX,
          posY: dot.posY,
          dotcolor: this.dots_bg,
          bgcolor: this.bg_color,
          textcolor: this.text_color,
          productID: dot.product.id,
        }));

        // Call createCollection method with imageUrl and the new array
        const response = await CollectionService.createCollection(
          this.uploadedImage,
          dataToSend
        );
        this.uploadedImage = null; // Reset uploaded image
        this.dotsArr = []; // Clear dots array
        this.dataSteps.currentStep = 0; // Return to the first step
    ...
    handleHotspotAdded(dots) {
      ...
      this.dotsArr.forEach((dot, index) => {
        ...
        console.log("Text:", dot.text);
        console.log("Position X:", dot.posX);
        console.log("Position Y:", dot.posY);
        console.log("Color:", dot.color);
        console.log("Type:", dot.type);
        console.log("Product:", dot.product);
        console.log("Filtered Products:", dot.filteredProducts);
        console.log("-------------------------");
      });
    },
