          ...
          >
            ...
            <div class="px-3 py-2">
              <input
                ...
                v-model="dot.text"
                @input="filterProducts($event, dot)"
                @change="changeDot(dot, i)"
                :style="inputStylesObj"
              />
              <div v-if="dot.filteredProducts.length" class="search-results">
                <ul class="py-2 text-sm text-gray-700 dark:text-gray-200">
                  <li
                    v-for="(product, index) in dot.filteredProducts"
                    :key="index"
                    @click="selectProduct(product, dot)"
...
export default {
  ...
  methods: {
    selectProduct(product, dot) {
      // Create a new dot object with updated properties
      const updatedDot = {
        ...dot,
        text: product.name,
        product: product,
      ...
      // Replace the dot in the dots array
      const index = this.dots.indexOf(dot);
      if (index !== -1) {
        this.dots.splice(index, 1, updatedDot);
      }

      this.$emit("hotspot-added", this.dots);
    },

    filterProducts(event, dot) {
      dot.filteredProducts = this.products.filter((product) => {
        return product.name
          .toLowerCase()
